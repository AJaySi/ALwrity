...
export const getTrustedOAuthOrigins = (provider: OAuthProvider, backendOrigins: string[] = []): string[] => {
...
export const getOAuthPostMessageTargetOrigin = (
  ...
  const trustedOrigins = getTrustedOAuthOrigins(provider, backendOrigins);
  const storedOrigin = getStoredOAuthTargetOrigin(provider);
